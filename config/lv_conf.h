/*
 * NEVOS LVGL configuration.
 *
 * Derived from third_party/lvgl/lv_conf_template.h at v9.2.2 and modified only
 * where NEVOS needs something different. Regenerate from the template on an
 * LVGL upgrade rather than merging by hand.
 *
 * NEVOS-specific choices:
 *   LV_COLOR_DEPTH 16      RGB565, matching the panel; no conversion in flush
 *   LV_MEM_SIZE 256 KB     a declared, bounded budget (see BUDGET.md) rather
 *                          than an unbounded clib heap
 *   LV_DEF_REFR_PERIOD 33  the 30 fps frame budget from ARCHITECTURE.md §5
 *   Montserrat 28 and 48   enabled for the shell's type scale
 *
 * The tick source is installed at runtime with lv_tick_set_cb(nev_now_ms), so
 * LVGL's clock is the same monotonic clock the rest of NEVOS uses.
 */
/**
 * @file lv_conf.h
